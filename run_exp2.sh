# Analyze all the rater and rerater results.  Create the CSV file we need
# for visualization.

language=english
dbfile=exp2/experiments_exp2_${language}.db
dir=exp2/exp_${language}_results
...
python offline_asr.py --dbfile $dbfile  \
  --audiodir=uploads --num_workers 6  --model=large \
  --language=en --debug > $dir/offline_asr.log
