...
def mangle(namespace):
    if not isinstance(namespace, str):
        namespace.mangle = True
        return namespace
    def decorator(cls):
        cls.mangled = namespace
        for i, j in zip(cls.__mro__[:-1], cls.__mro__[1:]):
            for k in dir(i):
                ...
                if v == getattr(j, k, object()) or not hasattr(v, "mangle"):
                    continue
                def closure(f):
                    mangled = namespace + "_" + f.__name__
                    ...
                    def router(self, *a, **kw):
                        if hasattr(self, "mangler"):
                            prefix = self.mangler.mangled + "_" + f.__name__
                            return getattr(self, prefix)(*a, **kw)
                        return f(self, *a, **kw)
...
print('child0_a')
obj.child0_a() # 0 Child0 child0
print('\nchild0_b')
obj.child0_b() # 3 0 Child0 child0
print('\nchild1_a')
obj.child1_a() # 2 1 0 Child1 child1
print('\nchild1_b')
obj.child1_b() # 3 2 1 0 Child1 child1
print()
'''
