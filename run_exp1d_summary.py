# Summarize the results from the run_exp1[de].sh script by parsing the 
...
flags.DEFINE_string('baseline', '', 'Which directory to get the baseline results.')
# Replace XXX wth test-type.
flags.DEFINE_string('output_plot', 'summary_XXX_plot.png', 'Filename to save the generated plot.')
flags.DEFINE_enum('test_type', 'forced', {'forced', 'exact'}, 'Type of test to summarize.')
...
def main(argv):
    ...
    if FLAGS.baseline:
        # If a baseline directory is provided, extract the baseline CNC and Win values
        baseline_pattern = os.path.join(FLAGS.baseline, 'analysis.txt')
        if os.path.exists(baseline_pattern):
            baseline_cnc, baseline_win = parse_metrics(baseline_pattern)
            if baseline_cnc is not None and baseline_win is not None:
                logging.info(f"Baseline CNC: {baseline_cnc}, Win: {baseline_win}")
                # Add the baseline 
                data.insert(0, ('Baseline', baseline_cnc, baseline_win))
            else:
                logging.warning(f"Could not extract valid metrics from baseline file: {baseline_pattern}")
        else:
